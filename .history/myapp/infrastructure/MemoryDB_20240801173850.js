...
class MemoryDB {
  ...
  fetchAccounts(year, month, sort, order) {
    ...
    if (sort === "date") {
      if (order === "desc") {
        data.sort((a, b) => new Date(b.date + "T" + b.time + ":00") - new Date(a.date + "T" + b.time + ":00"));
        return data;
      } else {
        data.sort((a, b) => new Date(a.date + "T" + b.time + ":00") - new Date(b.date + "T" + b.time + ":00"));
        return data;
      }
    }
  ...
  init() {
    ...
    statement2.add("food", 3000, "expension");
